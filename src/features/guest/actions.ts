...
/**
 * Get available items by category (quantity > 0 shown, quantity = 0 disabled).
 */
...
/**
 * Get all items for loan selection (all categories, quantity > 0).
 */
...
const createPickupSchema = z
  .object({
    ...
    purpose: z.string().min(1).max(200).trim()
  })
  .strict();
 ...
 * Atomically: insert pickup + decrement item quantity.
 */
